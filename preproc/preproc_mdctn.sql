/* assuming that we have a table mdctn_code_to_mdctn_name(concept_cd text, mdctn_name text) 
                                 ...
                                 mdctn_name_to_rxnorm(mdctn_name text, rxnorm text) */
...
create table mdctn_rxnorm as
  select patient_num,
         encounter_num,
	 (rxnorm || '_' || gene || '_' || instance_num) as concept,
	 modifier_cd as modifier,
	 valtype_cd as valtype,
	 valueflag_cd as valueflag,
	 nval_num as nval,
	 ...
	 start_date,
	 end_date
  from observation_fact inner join (mdctn_code_to_mdctn_name inner join mdctn_name_to_gene using (mdctn_name) inner join mdctn_name_to_rxnorm using (mdctn_name)) using (concept_cd);

select longtowide('mdctn_rxnorm', ARRAY['encounter_num','patient_num'], ARRAY['integer','integer'], 'concept',
                  ARRAY['valtype','nval','tval','units','start_date','end_date','modifier','valueflag'],
		  ARRAY['varchar(50)', 'numeric', 'varchar(255)','varchar(50)', 'timestamp', 'timestamp','varchar(100)','varchar(50)'],
		  ARRAY['mdctn_valtype','mdctn_nval','mdctn_tval','mdctn_units','mdctn_start_date','mdctn_end_date','mdctn_modifier','mdctn_valueflag'], 'mdctn_rxnorm_wide');
						      
drop table if exists features_rxnorm_wide;
create table features_rxnorm_wide as
  select *
  from visit_reduced
  full outer join icd_norm_wide using (patient_num, encounter_num)
  full outer join loinc_wide using (patient_num, encounter_num)
  full outer join mdctn_rxnorm_wide using (patient_num, encounter_num)
  full outer join vital_wide using (patient_num, encounter_num)
  inner join features using (patient_num);

create temp table tmp (like features_rxnorm_wide);
copy tmp to '/tmp/endotype_rxnorm_meta.csv' delimiter '!' csv header;
drop table tmp;
copy features_rxnorm_wide to '/tmp/endotype_rxnorm.csv' delimiter '!' null '';
copy loinc_wide_meta to '/tmp/loinc_rxnorm_meta.csv' delimiter '!';
copy mdctn_rxnorm_wide_meta to '/tmp/mdctn_rxnorm_meta.csv' delimiter '!';
copy vital_wide_meta to '/tmp/vital_rxnorm_meta.csv' delimiter '!';
copy icd_norm_wide_meta to '/tmp/icd_rxnorm_meta.csv' delimiter '!';

drop table if exists features_filter_trunc_rxnorm_wide;
create table features_filter_trunc_rxnorm_wide as
  select *
  from visit_reduced
  full outer join icd_filter_trunc_norm_wide using (patient_num, encounter_num)
  full outer join loinc_filter_wide using (patient_num, encounter_num)
...
create temp table tmp (like features_filter_trunc_rxnorm_wide);
copy tmp to '/tmp/endotype_filter_trunc_rxnorm_meta.csv' delimiter '!' csv header;
drop table tmp;
copy features_filter_trunc_rxnorm_wide to '/tmp/endotype_filter_trunc_rxnorm.csv' delimiter '!' null '';
copy loinc_filter_wide_meta to '/tmp/loinc_filter_trunc_rxnorm_meta.csv' delimiter '!';
copy mdctn_rxnorm_wide_meta to '/tmp/mdctn_filter_trunc_rxnorm_meta.csv' delimiter '!';
copy vital_wide_meta to '/tmp/vital_filter_trunc_rxnorm_meta.csv' delimiter '!';
copy icd_filter_trunc_norm_wide_meta to '/tmp/icd_filter_trunc_rxnorm_meta.csv' delimiter '!';
