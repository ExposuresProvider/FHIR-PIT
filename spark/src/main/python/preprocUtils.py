...
def preprocSocial(df):
    ...
    df["CAFO_Distance"], binsadd = pd.cut(df["CAFO_Distance"].astype(float), [0, 500, 1000, 2000, 4000, float("inf")], labels=list(map(str, [1, 2, 3, 4, 5])), include_lowest=True, right=False, retbins=True)
    bins += [("CAFO_Distance", binsadd.tolist())]
    df["LandfillDistance"], binsadd = pd.cut(df["LandfillDistance"].astype(float), [0, 500, 1000, 2000, 4000, float("inf")], labels=list(map(str, [1, 2, 3, 4, 5])), include_lowest=True, right=False, retbins=True)
    bins += [("LandfillDistance", binsadd.tolist())]
