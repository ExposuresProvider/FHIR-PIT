    ...
    time {
      ...
      for(rowdir <- rowdirs) {
        ...
        for (coldir <- coldirs) {
          ...
          writeToFile(hc, output_filename, "a,o3,pmij\n")
          appendToFile(hc, output_filename, output_filename_no_header)

          output_dir_fs.delete(output_file_path_no_header, false)
        }
      }
