...
object GeoidFinder {
  ...
  def createPointLCC(lat : Double, lon : Double) : Point = {
    ...
    gf.createPoint(targetCoordinate)
...
class GeoidFinder(blockgrpShapefilePath : String, geoidPrefix: String = "")  {
  ...
  private val index = new SpatialIndexFeatureCollection(features.getSchema)
  ...
  def getGeoidForLatLon(lat : Double, lon : Double) : Option[String] = {
    ...
    try {
      p = GeoidFinder.createPointLCC(lat, lon)
