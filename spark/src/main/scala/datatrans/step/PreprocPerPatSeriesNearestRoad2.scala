    ...
    time {
      ...
      if(output_file_file_system.exists(output_file_path)) {
        ...
        val rows = pddf0.mapPartitions(partition => {
          ...
          partition.map(r => {
            (r.getString(0), nearestRoad.getMinimumDistance(r.getString(1).toDouble, r.getString(2).toDouble), nearestRoad.getMatchedRoadType.get, nearestRoad.getMatchedRouteId, nearestRoad.getMatchedSpeed.get, nearestRoad.getMatchedNumLanes)
          })
        })
