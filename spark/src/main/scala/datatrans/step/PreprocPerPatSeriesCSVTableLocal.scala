  ...
  def step(spark: SparkSession, config:PreprocPerPatSeriesCSVTableConfig) : Unit = {
    ...
    time {
      ...
      try {
        ...
        val exitValue = Process(f"${tempDir.toString}/venv/bin/python src/main/python/perPatSeriesCSVTable.py --n_jobs 32 --config ${tempFi.toString}").!(ProcessLogger(println, System.err.println))
        if (exitValue < 0) {
