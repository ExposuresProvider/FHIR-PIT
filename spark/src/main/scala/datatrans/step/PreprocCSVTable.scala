...
//        var df_patient_combined = spark.read.format("csv").option("header", value = true).load(output_file_patient_combined)
        ...
        // Map HPMS/TL road distance columns (1 per release year of each) to a single column by choosing
        // the column corresponding to the most recent release year before/up to the study's end date
        mapper.nearest_road_map_map.values.map{ x => 
          val distance_feature_names = x.getDistanceFeatureNames
          val selectDistanceFeatureValue = udf((end_date: String, values: Seq[Double]) => {
            val study_end_joda = DateTime.parse(end_date, ISODateTimeFormat.dateParser())
            val (release_year, feature_name_for_release_year) = x.getDistanceFeatureForPeriod(study_end_joda)
            val idx = distance_feature_names.keys.toSeq.indexWhere{_ == release_year}
            values(idx)
          })
          df_all_visit = df_all_visit.withColumn(
            ...
            selectDistanceFeatureValue(
              $"end_date",
              array(distance_feature_names.values.toSeq.map(col): _*)
            )
          )
        }
