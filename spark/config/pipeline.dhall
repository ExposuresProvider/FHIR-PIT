...
let nearestRoadHPMSStep = λ(skip : Text) → Step.NearestRoad {
  ...
  step = {
    ...
    arguments = {
      patgeo_data = patgeo_output_path,
      nearestroad_data = "${basedirinput}/other/spatial/nearestRoadHPMS/hpms_major_roads_2017.shp",
      maximum_search_radius = Integer/toDouble (Natural/toInteger 500),
...
let binICEESStep = \(skip : Text) -> \(study_periods : List Text) -> Step.System {
    ...
    step = {
        ...
        arguments = {
            pyexec = pyexec,
            requirements = requirements,
            command = ["src/main/python/preprocBinning.py", feature_map_path, "${basediroutput}/icees2_xwalk", "${basediroutput}/icees2_bins"] # study_periods,
            workdir = "."
        }
    }
}
